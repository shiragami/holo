...
void read_mask(char *inputfile,unsigned char *Data, int length)
{
	...
	FILE *ifptr;
	ifptr = fopen(inputfile,"rb");
	if(ifptr == NULL) printf("Error opening the mask\n");
...
int main(int argc,char *argv[]) {  
	...
	if(argc==3){
		...
		printf("Error.\nMiguel <in.dat> <out.dat> [mask.raw]\n");
		return 0;
	}
	...
	WrappedImage = (float *) calloc(image_size, sizeof(float));
        /* Read input phase */
	read_data(argv[1], WrappedImage, image_size);

        /* Input mask image if specified */
	...
	unsigned char *extended_mask = (unsigned char *) calloc(image_size, sizeof(unsigned char));
        ...
        if(mask){
	    read_mask(argv[3], input_mask, image_size);
        }else{
	...
	write_data(argv[2], UnwrappedImage, image_size);
	free(UnwrappedImage);
	free(WrappedImage);
	free(input_mask);
	free(extended_mask);
